...
router.post("/updateUser", jwt, async ctx => {
    ...
    switch (true) {
        case Boolean(obj.password):
            ...
            if (!status) {
                ...
                ctx.session.user.password = obj.newPassword
    ...
    for (let o in result) {
        console.log(o)
        o !== "password" && (ctx.session.user[o] = result[o])
    }

    let token = await generateToken(result)
    ctx.session.token = token
    ctx.body = ctx.state.sendFrontEnd({ msg: '恭喜你，修改成功', token, user: ctx.session.user })
})
